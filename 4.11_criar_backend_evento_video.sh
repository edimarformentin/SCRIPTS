...
echo "----------------------------------------------------------------"
echo "==== SCRIPT 4.11 (v2): INSTALAR ASSEMBLER DE VÍDEO (NOVO LOCAL) ===="
echo "----------------------------------------------------------------"

CORE_SCRIPTS_DIR="/home/edimar/SISTEMA/GESTAO_WEB/core_scripts"
PY_SCRIPT="$CORE_SCRIPTS_DIR/event_assembler_host.py"
SERVICE="/etc/systemd/system/event-assembler.service"
TIMER="/etc/systemd/system/event-assembler.timer"
...
echo "[4.11] Gravando/atualizando $PY_SCRIPT ..."
cat > "$PY_SCRIPT" <<'PYEOF'
#!/usr/bin/env python3
...
echo "[4.11] Criando service/timer systemd (apontando para o novo local)..."
sudo bash -c "cat > '$SERVICE' <<EOF2
[Unit]
Description=Assembler de vídeo por evento (concatena gravações do Frigate)
...
ExecStart=/usr/bin/python3 ${PY_SCRIPT} --limit 0
Nice=10
...
[Unit]
Description=Timer - assembler de vídeo por evento
...
echo "[4.11] Recarregando, habilitando e iniciando o timer..."
sudo systemctl daemon-reload
sudo systemctl enable --now event-assembler.timer

echo "==== SCRIPT 4.11 (v2) CONCLUÍDO ===="
