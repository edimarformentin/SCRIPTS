...
# Script 04-frontend-02: Página de Câmeras (v5.5 - Copiar Path Melhorado)
#
# Implementa a funcionalidade de copiar o path da câmera para a área de
# transferência com feedback visual para o usuário.
# =================================================================

echo "--> 4.2: Criando a página de câmeras (v5.5 - Copiar Path Melhorado)..."
...
# --- cameras.js (COM A LÓGICA DE CÓPIA MELHORADA ) ---
echo "    -> Criando js/cameras.js (v5.5) com funcionalidade de cópia aprimorada..."
...
const App = {
    ...
    renderCameraList() {
        this.elements.cameraList.innerHTML = this.cameras.length === 0 ? '<p class="empty-list-message">Nenhuma câmera cadastrada.</p>' : this.cameras.map(cam => {
            ...
            // Para RTMP, o path a ser copiado não tem a barra inicial.
    ...
    handleListClick(e) {
        ...
        if (btn.classList.contains('copy-btn')) {
            navigator.clipboard.writeText(btn.dataset.path).then(() => {
                ...
                icon.classList.add('bi-check-lg'); // Ícone de "check"
                btn.classList.add('copied'); // Adiciona classe para feedback visual (opcional)
...
echo "--- Script 04-frontend-02 (v5.5) com funcionalidade de cópia aprimorada, atualizado com sucesso. ---"
