#!/bin/bash
# Nome do arquivo: 4.4_criar_backend_settings.sh
# Função: Cria o arquivo de configuração (settings.py) para o FastAPI.

set -Eeuo pipefail

echo "==== SCRIPT 4.4: CRIANDO ARQUIVO DE CONFIGURAÇÃO SETTINGS.PY ===="

# Navega para o diretório raiz do sistema.
cd /home/edimar/SISTEMA

echo "--> Gerando GESTAO_WEB/config/settings.py..."

# Usa 'cat' com 'EOF' para escrever o conteúdo do arquivo Python.
cat <<'SETTINGS_PY' > GESTAO_WEB/config/settings.py
...
echo "--> Arquivo config/settings.py criado com sucesso."
echo "==== SCRIPT 4.4 CONCLUÍDO ===="
