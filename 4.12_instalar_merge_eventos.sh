...
echo "----------------------------------------------------------------"
echo "==== SCRIPT 4.12 (v2): INSTALAR MERGE DE EVENTOS (NOVO LOCAL) ===="
echo "----------------------------------------------------------------"

CORE_SCRIPTS_DIR="/home/edimar/SISTEMA/GESTAO_WEB/core_scripts"
PY_SCRIPT="$CORE_SCRIPTS_DIR/event_merge_host.py"
SERVICE="/etc/systemd/system/event-merge.service"
TIMER="/etc/systemd/system/event-merge.timer"
mkdir -p "$CORE_SCRIPTS_DIR"

echo "[4.12] Gravando/atualizando $PY_SCRIPT ..."
cat > "$PY_SCRIPT" <<'PYEOF'
#!/usr/bin/env python3
...
echo "[4.12] Criando service/timer systemd (apontando para o novo local)..."
sudo bash -c "cat > '$SERVICE' <<'SVC'
[Unit]
Description=Merge de vídeos de eventos (concatena clipes próximos)
...
User=$USER
Group=$(id -gn $USER)
Environment=FRIGATE_BASE=/home/edimar/SISTEMA/FRIGATE
...
sudo bash -c "cat > '$TIMER' <<'TMR'
[Unit]
Description=Timer - merge de vídeos de eventos

[Timer]
OnBootSec=2min
OnUnitActiveSec=5min
Unit=event-merge.service
Persistent=true
...
echo "[4.12] Recarregando, habilitando e iniciando o timer..."
sudo systemctl daemon-reload
sudo systemctl enable --now event-merge.timer

echo "==== SCRIPT 4.12 (v2) CONCLUÍDO ===="
