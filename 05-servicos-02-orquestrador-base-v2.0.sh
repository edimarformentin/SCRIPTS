#!/bin/bash
# =================================================================
# Script: Orquestrador - Estrutura Base (v2.0 - Correção Final de Tipo)
#
# Garante que a variável de paths do MediaMTX seja sempre um
# dicionário, corrigindo o erro "'list' object has no attribute 'keys'".
# =================================================================
source "/home/edimar/SCRIPTS/00-configuracao-central.sh"

echo "--> 5.2: Criando a estrutura base do Orquestrador (v2.0 - Correção Final)..."
...
# --- orchestrator.py (LÓGICA FINAL E ROBUSTA) ---
...
def sync_cameras():
    print("="*50)
    print(f"[{time.ctime()}] Iniciando ciclo de sincronização v2.0...")
    try:
        # 1. Obter câmeras da API de Gestão
        ...
        if not isinstance(mtx_paths, dict):
            print(f"AVISO: a API do MediaMTX retornou um tipo inesperado para 'items'. Tratando como vazio.")
            mtx_paths = {}

        print(f"MediaMTX: {len(mtx_paths)} paths configurados.")

        # 3. Sincronizar Câmeras
        ...
        for name in all_managed_names:
            ...
            # Atualizar
                ...
                if current_source != expected_source:
                    print(f"  [UPDATE] Path '{name}' com URL incorreta. Corrigindo...")
...
if __name__ == "__main__":
    print("Serviço Orquestrador v2.0 (Robusto) iniciado.")
...
echo "--- Estrutura base do Orquestrador (v2.0) com correção final, criada com sucesso."
