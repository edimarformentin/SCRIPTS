...
# Script 04-frontend-02: Página de Câmeras (v5.0 - Refatoração da IA)
#
# 1. Unifica os selects de IA dos formulários de cadastro com um único ID.
# 2. Refatora o JS para usar uma única instância do Choices.js para cadastro.
# 3. Garante que a seleção de IA seja limpa após cada cadastro, corrigindo o bug.
# =================================================================

echo "--> 4.2: Criando a página de câmeras (v5.0 - Refatoração da IA)..."
mkdir -p "$FRONTEND_DIR/js"

# --- cameras.html (COM IDs DOS SELECTS DE IA UNIFICADOS) ---
echo "    -> Criando cameras.html (v5.0) com IDs de IA unificados..."
    ...
    <main class="container">
        ...
        <div id="rtmp" class="tab-content active">
            ...
            <form id="rtmp-form">
                ...
                <!-- ID UNIFICADO -->
                <div class="form-group"><label for="add-ia-select">Detecção com IA</label><select id="add-ia-select" multiple></select></div>
...
# --- cameras.js (COM A LÓGICA DE IA REATORADA ) ---
echo "    -> Criando js/cameras.js (v5.0) com lógica de IA refatorada..."
...
const App = {
    ...
    initializeChoices() {
        ...
        // Apenas DUAS instâncias: uma para ADICIONAR e outra para EDITAR.
        this.choicesInstances.add = new Choices('#add-ia-select', config);
        this.choicesInstances.edit = new Choices('#edit-ia-select', config);
        // Popula ambas com as opções de IA.
    ...
    async handleAddFormSubmit(e) {
        ...
        try {
            ...
            form.reset();
            // Limpa a instância de IA para o próximo cadastro.
            this.choicesInstances.add.clearStore();
            await this.refreshPageData();
        } catch (err) {
...
echo "--- Script 04-frontend-02 (v5.0) com refatoração da IA, atualizado com sucesso. ---"
