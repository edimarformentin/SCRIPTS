#!/bin/bash
# =================================================================
# Script 06-database-01: Seed de Dados (v2.2 - Correção de Execução)
#
# Propósito:
# Popula o banco de dados com clientes e câmeras de teste.
#
# Correção nesta versão:
# Remove a barra invertida de escape na chamada do psql, garantindo
# que a variável $SQL_COMMAND seja corretamente expandida pelo shell
# e que o conteúdo SQL seja de fato executado pelo banco de dados.
# =================================================================
set -e
source "/home/edimar/SCRIPTS/00-configuracao-central.sh"

echo "--> 6.1: Populando o banco com dados de teste via INSERT direto (seeding v2.2)..."
...
('$CLIENTE2_ID', 'cam_externa', 'rtsp://admin:b1n2m32019@187.85.161.250:55533/mode=real&idc=1&ids=1', 1, true, true)
...
# AQUI ESTÁ A CORREÇÃO: removido o '\' antes de $SQL_COMMAND
docker exec -i "$DB_CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" <<< "$SQL_COMMAND"

echo "--- Seed de dados (v2.2) via INSERT direto concluído com sucesso. ---"
