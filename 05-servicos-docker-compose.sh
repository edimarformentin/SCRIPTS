#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# 05-servicos-docker-compose.sh  (compose v2 + PGDATA subdir + nginx sem log mount)
# -----------------------------------------------------------------------------
set -Eeuo pipefail
source /home/edimar/SCRIPTS/00-configuracao-central.sh

main() {
  mkdir -p "$DATA_DIR/postgres" "$DATA_DIR/rabbitmq" "$DATA_DIR/mediamtx" \
           "$FRONTEND_DIR/public" "$CONFIG_DIR/nginx" "$CONFIG_DIR/mediamtx" \
...
YML
  ...
  ok "docker-compose.yml gerado (nginx sem volume de logs)."
}
main "$@"
