...
# Script 04-frontend-02: Página de Câmeras (v5.6 - Cópia Segura e Compatível)
#
# Corrige o erro "Cannot read properties of undefined" ao verificar se a API
# navigator.clipboard está disponível. Se não estiver (contexto inseguro),
# usa um método de cópia legado como fallback.
# =================================================================

echo "--> 4.2: Criando a página de câmeras (v5.6 - Cópia Segura e Compatível)..."
mkdir -p "$FRONTEND_DIR/js"

# --- Recria o cameras.html (v5.3) que já estava correto ---
echo "    -> Recriando cameras.html (v5.3)..."
    ...
    <main class="container">
        ...
        <div id="rtmp" class="tab-content active">
            ...
            <form id="rtmp-form">
                ...
                <div class="form-group"><label for="rtmp-gravacao">Gravação</label><select id="rtmp-gravacao">
                ...
                <div class="form-group"><label for="add-ia-select-rtmp">Detecção com IA</label><select id="add-ia-select-rtmp" multiple></select></div>
        ...
        <div id="rtsp" class="tab-content">
            ...
            <form id="rtsp-form">
                ...
                <div class="form-group"><label for="rtsp-gravacao">Gravação</label><select id="rtsp-gravacao">
                    <option value="1" selected>1 dia</option>
                    <option value="2">2 dias</option>
                    <option value="3">3 dias</option>
                    <option value="4">4 dias</option>
                ...
                <div class="form-group"><label for="add-ia-select-rtsp">Detecção com IA</label><select id="add-ia-select-rtsp" multiple></select></div>
    ...
    <div id="edit-camera-modal" class="modal">
        <div class="modal-content">
            ...
            <form id="edit-camera-form">
                ...
                <div class="form-group"><label for="edit-gravacao">Gravação</label><select id="edit-gravacao">
...
# --- cameras.js (COM A LÓGICA DE CÓPIA SEGURA ) ---
echo "    -> Criando js/cameras.js (v5.6) com lógica de cópia segura e compatível..."
...
const App = {
    ...
    renderCameraList() {
        this.elements.cameraList.innerHTML = this.cameras.length === 0 ? '<p class="empty-list-message">Nenhuma câmera cadastrada.</p>' : this.cameras.map(cam => {
            ...
            const pathToCopy = isRtsp ? cam.url_rtsp : cam.url_rtmp_path;
            ...
            return `<div class="camera-list-item" data-camera-id="${cam.id}">
                        ...
                        <div class="item-actions">
                            <button class="icon-btn copy-btn" title="Copiar Path" data-path="${pathToCopy}"><i class="bi bi-clipboard"></i></button>
    ...
    async handleAddFormSubmit(e) {
        ...
        const data = {
            ...
            nome_camera: form.querySelector(`#${type}-nome`).value,
            dias_gravacao: parseInt(form.querySelector(`#${type}-gravacao`).value),
            ...iaSelections,
            url_rtsp: type === 'rtsp' ? form.querySelector('#rtsp-url').value : null
        };
        try {
            await API.Camera.create(data);
            form.reset();
            this.choicesInstances.add.removeActiveItems();
            await this.refreshPageData();
        } catch (err) {
    ...
    handleListClick(e) {
        ...
        if (btn.classList.contains('copy-btn')) {
            this.copyToClipboard(btn.dataset.path, btn);
        } else if (btn.classList.contains('edit')) {
            this.openEditModal(camId);
        } else if (btn.classList.contains('delete')) {
            this.deleteCamera(camId);
        }
    },
    copyToClipboard(text, btnElement) {
        ...
        // Usa a API moderna se disponível (contexto seguro: HTTPS ou localhost)
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(text).then(showFeedback);
        } else {
            // Fallback para o método legado (contexto inseguro: HTTP)
...
echo "--- Script 04-frontend-02 (v5.6) com cópia segura e compatível, atualizado com sucesso. ---"
