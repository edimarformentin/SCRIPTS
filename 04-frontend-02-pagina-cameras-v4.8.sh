...
# Script 04-frontend-02: Página de Câmeras (v4.8 - Sugestão de Nomes para RTSP)
#
# Aprimora a função de sugestão de nomes para que ela também preencha
# automaticamente o nome da câmera no formulário RTSP.
# =================================================================

echo "--> 4.2: Criando a página de câmeras (v4.8 - Sugestão de Nomes para RTSP)..."
...
# --- cameras.js (COM A LÓGICA DE SUGESTÃO DE NOMES ATUALIZADA ) ---
echo "    -> Criando js/cameras.js (v4.8) com sugestão de nomes para RTSP..."
...
const App = {
    ...
    initializeChoices() {
        ...
        this.choicesInstances.rtmp.setChoices(this.iaOptions, 'value', 'label', false);
        this.choicesInstances.rtsp.setChoices(this.iaOptions, 'value', 'label', false);
        this.choicesInstances.edit.setChoices(this.iaOptions, 'value', 'label', false);
    },
    setupEventListeners() {
    ...
    // --- FUNÇÃO ATUALIZADA ---
    suggestCamNames() {
        const rtmpNames = new Set(this.cameras.filter(c => c.url_rtmp_path).map(c => c.nome_camera));
        const rtspNames = new Set(this.cameras.filter(c => c.url_rtsp).map(c => c.nome_camera));

        let rtmpCounter = 1;
        let suggestedRtmpName;
        do {
            suggestedRtmpName = `cam_rtmp_${rtmpCounter++}`;
        } while (rtmpNames.has(suggestedRtmpName));
        this.elements.rtmpForm.querySelector('#rtmp-nome').value = suggestedRtmpName;

        let rtspCounter = 1;
        let suggestedRtspName;
        do {
            suggestedRtspName = `cam_rtsp_${rtspCounter++}`;
        } while (rtspNames.has(suggestedRtspName));
        this.elements.rtspForm.querySelector('#rtsp-nome').value = suggestedRtspName;
    },
...
echo "--- Script 04-frontend-02 (v4.8) com sugestão de nomes para RTSP, atualizado com sucesso. ---"
