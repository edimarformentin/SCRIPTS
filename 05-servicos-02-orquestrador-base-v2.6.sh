...
# Script 05-servicos-02: Orquestrador (v2.6 - Lógica de Upsert)
#
# Implementa uma lógica de "Upsert" (tenta PATCH, se 404, faz POST)
# para garantir a criação e atualização de paths no MediaMTX.
# =================================================================

echo "--> 5.2: Criando a estrutura do Orquestrador (v2.6 - Lógica de Upsert)..."
...
echo "    -> Criando orchestrator.py com lógica de Upsert para o MediaMTX..."
...
def sync_mediamtx(api_rtsp_cameras, mtx_paths):
    ...
    for name in all_managed_names:
        ...
        if cam_in_db and cam_in_db.get('is_active'):
            print(f"  [SYNC] Garantindo path '{name}' no MediaMTX...")
...
def main_cycle():
    print("="*60)
    print(f"[{time.ctime()}] Iniciando ciclo de orquestração v2.6...")
    try:
...
if __name__ == "__main__":
    print("Serviço Orquestrador v2.6 (Upsert) iniciado.")
...
echo "--- Estrutura do Orquestrador (v2.6) criada com sucesso."
