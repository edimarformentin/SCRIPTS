...
# Script 04-frontend-02: Página de Câmeras (v4.7 - Script JS Corrigido)
#
# Corrige o HTML da página de câmeras para carregar o 'cameras.js'
# em vez do 'app.js', resolvendo o conflito de scripts.
# =================================================================

echo "--> 4.2: Criando a página de câmeras (v4.7 - Script JS Corrigido)..."
mkdir -p "$FRONTEND_DIR/js"

# --- cameras.html (COM A TAG SCRIPT CORRIGIDA) ---
echo "    -> Criando cameras.html com a tag de script correta..."
...
<body>
    ...
    <main class="container">
        ...
        <div id="rtmp" class="tab-content active">
            ...
            <form id="rtmp-form">
                <div class="form-group"><label for="rtmp-nome">Nome da Câmera</label><input type="text" id="rtmp-nome" required maxlength="10" pattern="[a-zA-Z0-9_]+"><small>Até 10 caracteres, sem espaços ou especiais.</small></div>
...
# --- Recria o cameras.js (v4.6 ) que já estava correto ---
echo "    -> Recriando js/cameras.js (v4.6)..."
...
const App = {
    ...
    suggestRtmpCamName() {
        const rtmpNames = new Set(this.cameras.filter(c => !c.url_rtsp).map(c => c.nome_camera));
        let i = 1, suggestedName;
        do { suggestedName = `cam${i++}`; } while (rtmpNames.has(suggestedName));
...
echo "--- Script 04-frontend-02 (v4.7) com script JS corrigido, atualizado com sucesso. ---"
