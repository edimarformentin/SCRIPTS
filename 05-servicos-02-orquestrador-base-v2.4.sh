...
# Script 05-servicos-02: Orquestrador (v2.4 - Correção de Tipo e Publicador)
#
# Corrige o erro "'list' object has no attribute 'items'" e
# publica tarefas para as câmeras ativas no RabbitMQ.
# =================================================================

echo "--> 5.2: Criando a estrutura do Orquestrador (v2.4 - Correção e Publicador)..."
ORQUESTRADOR_DIR="$SISTEMA_DIR/ORQUESTRADOR"
mkdir -p "$ORQUESTRADOR_DIR"

# --- requirements.txt: Adiciona 'pika' ---
echo "    -> Adicionando 'pika' aos requirements do orquestrador..."
...
# --- orchestrator.py (COM A LÓGICA CORRIGIDA E DE PUBLICAÇÃO) ---
echo "    -> Criando orchestrator.py com correção e lógica de publicação..."
...
def publish_task(camera):
    ...
    try:
        ...
        message = {
            ...
            "camera_name": camera.get('nome_camera'),
            "rtsp_url": camera.get('url_rtsp'),
            "rtmp_path": camera.get('url_rtmp_path')
        }
...
def main_cycle():
    print("="*60)
    print(f"[{time.ctime()}] Iniciando ciclo de orquestração v2.4...")
    try:
...
if __name__ == "__main__":
    print("Serviço Orquestrador v2.4 (Corrigido e Publicador) iniciado.")
...
echo "--- Estrutura do Orquestrador (v2.4) criada com sucesso."
