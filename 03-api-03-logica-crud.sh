...
echo "--> 3.3: Criando a lógica CRUD da API (v-unificada)..."
...
def generate_readable_id(db, name):
    base_slug = slugify(name.split()[0])
    while True:
        suffix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=6))
        readable_id = f"{base_slug}-{suffix}"
        with db.cursor() as cur:
            cur.execute("SELECT id FROM clientes WHERE id_legivel = %s;", (readable_id,))
            if cur.fetchone() is None: return readable_id
def create(db, *, client_in: ClientCreate):
...
def create(db, *, camera_in: CameraCreate):
    ...
    with db.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        db.commit()
        return new_camera
def update(db, *, camera_id: UUID, camera_in: CameraUpdate):
    update_data = camera_in.model_dump(exclude_unset=True)
    if not update_data: return get(db, camera_id=camera_id)
...
CRUD_CAM_EOF
echo "--- Lógica CRUD da API unificada."
